...
class MaterialUtils:
    @staticmethod
    def _get_node_tree_visit_key(node_tree):
        if node_tree is None:
            return None

        name_full = getattr(node_tree, "name_full", "")
        if name_full:
            return f"name:{name_full}"

        try:
            return f"ptr:{node_tree.as_pointer()}"
        except Exception:
            return f"id:{id(node_tree)}"

    @staticmethod
    def _iter_unique_materials(materials):
    ...
    @staticmethod
    def walk_material_node_trees(
        materials,
        mutate_node_tree=None,
        should_mutate_node_tree=None,
        duplicate_shared_groups=True,
    ):
        """Recursively walk material and nested group node trees.

        Args:
            materials: Iterable of ``bpy.types.Material``.
            mutate_node_tree: Optional callable ``(node_tree, depth) -> bool``.
                Return ``True`` when that node tree was modified.
            should_mutate_node_tree: Optional callable ``(node_tree, depth) -> bool``
                used as a pre-check for copy-on-write group duplication.
            duplicate_shared_groups: Duplicate shared group datablocks before
                mutation and rebind the owning ShaderNodeGroup node.

        Returns:
            ``list[bpy.types.NodeTree]`` with every modified node tree.
        """
        modified_node_trees = []
        modified_node_tree_keys = set()
        visited_node_tree_keys = set()

        def mark_modified(node_tree):
            node_tree_key = MaterialUtils._get_node_tree_visit_key(node_tree)
            if node_tree_key is None or node_tree_key in modified_node_tree_keys:
                return

            modified_node_tree_keys.add(node_tree_key)
            modified_node_trees.append(node_tree)

        def walk_node_tree(node_tree, depth=0, owner_group_node=None):
            if node_tree is None:
                return

            active_node_tree = node_tree
            node_tree_key = MaterialUtils._get_node_tree_visit_key(active_node_tree)

            if node_tree_key in visited_node_tree_keys:
                return

            should_attempt_mutation = mutate_node_tree is not None
            if should_attempt_mutation and should_mutate_node_tree is not None:
                try:
                    should_attempt_mutation = bool(
                        should_mutate_node_tree(active_node_tree, depth)
                    )
                except Exception:
                    should_attempt_mutation = True

            if (
                should_attempt_mutation
                and owner_group_node is not None
                and duplicate_shared_groups
                and getattr(active_node_tree, "users", 0) > 1
            ):
                copied_node_tree = active_node_tree.copy()
                owner_group_node.node_tree = copied_node_tree
                active_node_tree = copied_node_tree
                node_tree_key = MaterialUtils._get_node_tree_visit_key(active_node_tree)
                mark_modified(active_node_tree)

            if node_tree_key in visited_node_tree_keys:
                return

            visited_node_tree_keys.add(node_tree_key)

            if mutate_node_tree is not None and should_attempt_mutation:
                try:
                    if bool(mutate_node_tree(active_node_tree, depth)):
                        mark_modified(active_node_tree)
                except Exception:
                    pass
            ...
            for node in active_node_tree.nodes:
                ...
                walk_node_tree(
                    group_tree,
                    depth=depth + 1,
                    owner_group_node=node,
                )

        for material in MaterialUtils._iter_unique_materials(materials):
            if not getattr(material, "use_nodes", False):
                continue

            walk_node_tree(getattr(material, "node_tree", None), depth=0)

        return modified_node_trees

    @staticmethod
    def walk_object_material_node_trees(
        obj,
        mutate_node_tree=None,
        should_mutate_node_tree=None,
        duplicate_shared_groups=True,
        ...
        return MaterialUtils.walk_material_node_trees(
            materials=[slot.material for slot in obj.material_slots],
            mutate_node_tree=mutate_node_tree,
            should_mutate_node_tree=should_mutate_node_tree,
            duplicate_shared_groups=duplicate_shared_groups,
        )
