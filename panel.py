...
class GAMEREADY_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Quick output path (works for textures-only workflows too)
        output_row = action_box.row()
        output_row.enabled = not is_running
        output_row.prop(scene, "gameready_output_dir", text="Output Folder")
...
class GAMEREADY_PT_panel_base(bpy.types.Panel):
    ...
    def draw_export_settings(layout, scene, *, expanded):
        ...
        export_box.label(text="Export Model", icon="EXPORT")
        export_box.prop(scene, "gameready_export_files")

        # Output folder is used for textures and exports.
        output_sub = export_box.column()
        output_sub.enabled = scene.gameready_bake_textures or scene.gameready_export_files
        output_sub.prop(scene, "gameready_output_dir")
...
class GAMEREADY_PT_common_settings_panel(GAMEREADY_PT_panel_base):
    ...
    def draw(self, context):
        ...
        naming_box.label(text="Naming", icon="FILE_TEXT")
        naming_box.prop(scene, "gameready_use_custom_title")
        title_row = naming_box.row()
        title_row.enabled = scene.gameready_use_custom_title
        title_row.prop(scene, "gameready_custom_title")
