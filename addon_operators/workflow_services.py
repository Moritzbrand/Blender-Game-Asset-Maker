...
class GameAssetWorkflowServices:
    ...
    def make_source_materials_single_user(self, context):
        ...
        MaterialUtils.make_materials_single_user_for_bake_coordinate_nodes(source_object)

    def prepare_bake_coordinate_compensation(self, context):
        source_object = self.store.get_object(self.state.temporary_object_name)
        if source_object is None:
            source_object = self.store.get_object(self.state.game_asset_name)

        result = MaterialUtils.prepare_bake_coordinate_nodes(context, source_object)
        self.state.temporary_helper_object_names = result.helper_object_names
